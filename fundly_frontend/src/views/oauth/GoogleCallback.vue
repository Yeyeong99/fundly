...
onMounted(async () => {
  ...
  if (code) {
    try {
      const res = await axioInstance.post('http://127.0.0.1:8000/api/auth/google/login/', {
        code,
        redirect_uri: 'http://localhost:5173/api/auth/google/callback/', // ✅ Vue도 redirect_uri 명시
      })
